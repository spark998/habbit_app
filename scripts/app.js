...
const page = {
	...
	header: {
		...
		progressCoverBar: document.querySelector('.progrerss__cover-bar'),
	},
	content: {
...
function togglePopup() {
	/*if (page.popup.index.classList.conteins('cover_hidden')) {
	...
	for (const index in activeHabbit.days) {
		...
		element.innerHTML = ` <div class="habbit__day">День ${+index + 1}</div>
					...
					<button class="habbit__delete" onclick="removeDays(${+index})">
						<img class="img-btn" src="./images/delete.svg" alt="Удальть день ${+index + 1} ">
					</button>`;
...
(() => {
	...
	const urlHabbits = habbits.find(habbit => habbit.id === hashId);
	if (urlHabbits) {
		rerender(urlHabbits.id);
	} else {
